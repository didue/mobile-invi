...
- 현재 실제로 존재하는 파일은 `src/data/wedding.ts`(청첩장에 들어갈 실제 데이터: 신랑신부 정보, 연락처, 마음 전하실 곳 계좌, 갤러리 경로, 예식장 안내, 지도 링크)와 `.claude/rules/` 하위 문서뿐이다.
- 이전 CRA 소스(`src/App.js`, `src/pages/*.jsx`, `public/` 등)는 git 히스토리에는 남아 있지만 현재 워킹 트리에서는 삭제된 상태다. 필요하면 `git show HEAD:<path>`로 참고할 수 있다.
